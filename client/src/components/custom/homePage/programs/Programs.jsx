...
const FacultySection = () => {
  return (
    <div className="faculty py-16 bg-gray-50 dark:bg-black dark:text-white">
      ...
      <div className="flex flex-wrap justify-center gap-8">
        {facultyMembers.map((faculty, index) => (
          <motion.div
            key={index}
            className="w-64 p-4 bg-white dark:bg-gray-800 dark:text-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition duration-500"
          ...
          >
            <img
              src={faculty.image}
              alt={`Faculty member ${faculty.name}`}
              className="w-full h-48 object-cover rounded-t-xl"
            />
            <div className="text-center mt-4">
              <h3 className="text-2xl font-semibold text-gray-800 dark:text-white">{faculty.name}</h3>
              <p className="text-lg text-gray-500 dark:text-gray-300">{faculty.subject}</p>
            </div>
          </motion.div>
        ))}
      </div>
